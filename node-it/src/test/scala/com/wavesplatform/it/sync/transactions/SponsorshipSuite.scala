...
class SponsorshipSuite
  ...
  val sponsor           = firstKeyPair
  val alice             = secondKeyPair
  val bob               = thirdKeyPair
  ...
  private val aliceAddress: String = alice.toAddress.toString
  private val sponsorAddress: String = sponsor.toAddress.toString
  ...
  protected override def beforeAll(): Unit = {
    ...
    firstTransferTxToAlice = sender.transfer(sponsor, aliceAddress, sponsorAssetTotal / 2, minFee, Some(firstSponsorAssetId), None, waitForTx = true).id
    secondTransferTxToAlice = sender.transfer(sponsor, aliceAddress, sponsorAssetTotal / 2, minFee, Some(secondSponsorAssetId), None, waitForTx = true).id
  ...
  "Fee in sponsored asset works fine for transaction" - {
    ...
    "fee should be written off in issued asset" - {
      ...
      "alice transfer sponsored asset to bob using sponsored fee" in {
        val firstTransferTxCustomFeeAlice = sender.transfer(alice, bobAddress, 10 * Token, SmallFee, Some(firstSponsorAssetId), Some(firstSponsorAssetId)).id
    ...
    "set sponsopship again" - {
      ...
      "transfer tx sponsored fee is less then new minimal" in {
        ...
        assertBadRequestAndResponse(
          sender
            .transfer(sponsor, aliceAddress, 11 * Token, fee = SmallFee, assetId = Some(secondSponsorAssetId), feeAssetId = Some(secondSponsorAssetId))
            .id,
    ...
    "miner is sponsor" in {
      ...
      val secondMinersAsset =
        ...
        .id
      val firstSponsorshipTxId = miner.sponsorAsset(miner.keyPair, firstMinersAsset, baseFee = Token, fee = sponsorReducedFee, version = TxVersion.V1).id
      val secondSponsorshipTxId = miner.sponsorAsset(miner.keyPair, secondMinersAsset, baseFee = Token, fee = sponsorReducedFee, version = TxVersion.V2).id
